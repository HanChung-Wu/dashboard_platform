...
> ⚠️ **此倉庫仍處於開發階段，當前版本：`v0.2.0`，尚未正式發布。**
...
🔧 **最新進展（v0.2.0）**
- ⚠ **重大變更**：共用型別已移至 `shared/types`，請更新相關匯入。
- ✅ **資料表功能**
  - 支援 **編輯導覽**，點擊表格名稱可直接進入編輯頁。
  - 新增 **建立新表格** 功能。
  - 新增 **刪除確認對話框**。
  - 支援 **後端載入/更新資料表**，並統一資料模式。
- ✅ **React Hooks**
  - `useTableDataInitializer`：初始化資料。
  - `useTableGetter`：取得後端資料表內容與資訊。
- ✅ **UI & 佈局**
  - **RightPanel**：支援標題、可折疊、預設文字，並改善佈局。
- 🛠 **儀表板管理 UI** 開發中。
- 🛠 **互動式圖表功能** 開發中。
...
完整更新記錄請參見 [CHANGELOG.md](./CHANGELOG.md)
